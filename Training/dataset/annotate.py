...
def draw(img, xy, cfg, circles, score, color=(255, 255, 0)):
    ...
    font_scale = 0.5
    line_type = 1
    if xy.shape[0] > 7: # xy.shape[0] is the number of points
        xy = xy.reshape((-1, 2))
    if np.mean(xy) < 1:
        h, w = img.shape[:2]
        xy[:, 0] *= w
...
def get_bounding_box(img_path, scale=0.7):
    img = cv2.imread(img_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--img-folder', default='bw_dataset_2')
    parser.add_argument('-s', '--scale', type=float, default=1.0)
    parser.add_argument('-d', '--draw-circles', action='store_true')
    args = parser.parse_args()

    cfg = CN(new_allowed=True)
    cfg.merge_from_file('./configs/bw_darts.yaml')
